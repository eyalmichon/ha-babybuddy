...
def register(mcp) -> None:  # noqa: ANN001
    ...
    @mcp.tool
    async def ha_find_entities(pattern: str = "baby") -> list[dict[str, str]]:
        """Find all HA entities matching a pattern in their entity_id.

        Args:
            pattern: Text to match (case-insensitive, default 'baby').
        """
    ...
    @mcp.tool
    async def ha_integration_status(domain: str = "babybuddy") -> dict[str, Any]:
        """Get the status and options of an HA integration config entry.

        Args:
            domain: Integration domain (default 'babybuddy').
        """
    ...
    @mcp.tool
    async def ha_reload_integration() -> dict[str, Any]:
        """Reload the babybuddy config entry without restarting HA."""
        result = await api_request("GET", "/api/config/config_entries/entry")
        if "error" in result:
            return result
        ...
        return {"error": "No babybuddy config entry found"}
